...
public class GuiAddWaypoint extends GuiScreenMinimap implements IPopupGuiScreen {
    ...
    public void initGui() {
        ...
        this.doneButton = new PopupGuiButton(0, this.getWidth() / 2 - 155, this.getHeight() / 6 + 168, 150, 20, I18nUtils.getString("addServer.add"), this) {
            public void onClick(double p_onClick_1_, double p_onClick_3_) {
                GuiAddWaypoint.this.actionPerformed(this);
            }
        };
        this.addButton(this.doneButton);
        this.addButton(new PopupGuiButton(1, this.getWidth() / 2 + 5, this.getHeight() / 6 + 168, 150, 20, I18nUtils.getString("gui.cancel"), this) {
            public void onClick(double p_onClick_1_, double p_onClick_3_) {
                GuiAddWaypoint.this.actionPerformed(this);
            }
        });
        ...
        this.addButton(
                new PopupGuiButton(8, this.getWidth() / 2 - 101, buttonListY + 48, 100, 20, I18nUtils.getString("minimap.waypoints.sortbyicon") + ":     ", this) {
                    public void onClick(double p_onClick_1_, double p_onClick_3_) {
                        GuiAddWaypoint.this.actionPerformed(this);
                    }
                }
        );
        this.doneButton.enabled = this.waypointName.getText().length() > 0;
        ...
        if (par1GuiButton.enabled) {
            ...
            if (par1GuiButton.id == 1) {
                ...
                if (this.parentGui != null) {
                    this.parentGui.confirmResult(false, 0);
                } else {
                ...
                if (this.parentGui != null) {
                    this.parentGui.confirmResult(true, 0);
                } else {
                    if (this.editing) {
    ...
    public boolean keyPressed(int keysm, int scancode, int b) {
        if (!this.popupOpen()) {
            this.waypointName.keyPressed(keysm, scancode, b);
            this.waypointX.keyPressed(keysm, scancode, b);
            this.waypointZ.keyPressed(keysm, scancode, b);
            this.waypointY.keyPressed(keysm, scancode, b);
            if (keysm == 258) {
    ...
    public boolean charTyped(char character, int keycode) {
        if (!this.popupOpen()) {
            this.waypointName.charTyped(character, keycode);
            this.waypointX.charTyped(character, keycode);
            this.waypointZ.charTyped(character, keycode);
            this.waypointY.charTyped(character, keycode);
            boolean acceptable = this.waypointName.getText().length() > 0;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
        if (!this.popupOpen()) {
            ...
            float scScale = (float) this.mc.mainWindow.getGuiScaleFactor();
            ...
            if (displayWidthFloat > this.mc.mainWindow.getWidth()) {
                float adj = displayWidthFloat / this.mc.mainWindow.getWidth();
                scale *= adj;
                displayWidthFloat /= adj;
                displayHeightFloat /= adj;
            }

            if (displayHeightFloat > this.mc.mainWindow.getHeight()) {
                float adj = displayHeightFloat / this.mc.mainWindow.getHeight();
                scale *= adj;
                displayWidthFloat /= adj;
                displayHeightFloat /= adj;
            }
    ...
    public boolean mouseScrolled(double amount) {
        return this.popupOpen() || this.dimensionList == null || this.dimensionList.mouseScrolled(amount);
    }
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        this.waypointName.drawTextField(mouseX, mouseY, partialTicks);
        this.waypointX.drawTextField(mouseX, mouseY, partialTicks);
        this.waypointZ.drawTextField(mouseX, mouseY, partialTicks);
        this.waypointY.drawTextField(mouseX, mouseY, partialTicks);
        int buttonListY = this.getHeight() / 6 + 82 + 6;
        ...
        if (this.choosingIcon) {
            ...
            this.drawTexturedModalRect(
                    this.getWidth() / 2 - displayWidth / 2 - 1, this.getHeight() / 2 - displayHeight / 2 - 1, 0, 0, displayWidth + 2, displayHeight + 2
            );
            GLShim.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public void toggleDimensionSelected() {
        if (this.waypoint.dimensions.size() > 1
                ...
                && this.selectedDimension != this.master.getDimensionManager().getDimensionContainerByDimension(Minecraft.getInstance().world.dimension)) {
