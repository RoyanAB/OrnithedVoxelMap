	...
	public void initGui() {
		...
		for (EnumOptionsMinimap option : relevantOptions) {
			...
			GuiOptionButtonMinimap button = new GuiOptionButtonMinimap(
				option.returnEnumOrdinal(), leftBorder + optionIndex % 2 * 160, this.getHeight() / 6 + 24 * (optionIndex >> 1), option, text
			);
		...
		this.worldSeedButton = new GuiButtonText(
			this.worldSeedButtonID, this.getFontRenderer(), leftBorder + optionIndex % 2 * 160, this.getHeight() / 6 + 24 * (optionIndex >> 1), 150, 20, buttonText
		);
