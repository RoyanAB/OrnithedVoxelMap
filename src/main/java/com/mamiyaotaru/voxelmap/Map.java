...
public class Map implements Runnable, IMap {
    ...
    public void onTickInGame(Minecraft mc) {
        ...
        if (this.game.currentScreen == null && this.options.keyBindWaypoint.isPressed()) {
            ...
            Waypoint newWaypoint = new Waypoint(
                    "",
                    this.game.player.dimension != DimensionType.NETHER ? GameVariableAccessShim.xCoord() : GameVariableAccessShim.xCoord() * 8,
                    this.game.player.dimension != DimensionType.NETHER ? GameVariableAccessShim.zCoord() : GameVariableAccessShim.zCoord() * 8,
        ...
        this.enabled = !mc.gameSettings.hideGUI
    ...
    private void cycleZoomLevel() {
        if (this.options.zoom == 4) {
            ...
            if (this.multicore && this.game.gameSettings.getOptionFloatValue(Options.RENDER_DISTANCE) > 8.0) {
                this.options.zoom = 4;
                this.error = I18nUtils.getString("minimap.ui.zoomlevel") + " (0.25x)";
            } else {
                this.options.zoom = 3;
    ...
    private DynamicTexture getLightmapTexture() {
        Object lightTextureObj = ReflectionUtils.getPrivateFieldValueByType(this.game.gameRenderer, GameRenderer.class, LightTexture.class);
    ...
    public void calculateCurrentLightAndSkyColor() {
        if (this.world != null) {
            if (this.needLightmapRefresh && this.tickCounter != this.tickWithLightChange && !this.game.isGamePaused() || this.options.realTimeTorches) {
                GLUtils.disp(this.lightmapTexture.getGlTextureId());
            ...
            float[] providerLightBrightnessTable = this.world.dimension.getLightBrightnessTable();
            ...
            boolean scheduledUpdate = (this.timer - 50)
                    % (this.lastLightBrightnessTable[0] == 0.0F ? 250 : (this.game.player.dimension != DimensionType.NETHER ? 500 : 5000))
                    == 0;
            ...
            boolean aboveHorizon = this.game.player.getEyePosition(0.0F).y + this.game.player.getEyeHeight() >= this.world.getHorizon();
            if (DimensionType.getKey(this.world.dimension.getType()).toString().toLowerCase().contains("ether")) {
                aboveHorizon = true;
            }
    ...
    private int getSkyColor() {
        ...
        if (this.game.world.dimension.isSurfaceWorld()
                && this.game.gameSettings.getOptionFloatValue(Options.RENDER_DISTANCE) >= 4.0
            ...
            boolean showLocalFog = this.world.dimension.doesXZShowFog(GameVariableAccessShim.xCoord(), GameVariableAccessShim.zCoord());
            float farPlaneDistance = (float) (this.game.gameSettings.getOptionFloatValue(Options.RENDER_DISTANCE) * 16.0);
            float fogStart = 0.0F;
            float fogEnd = 0.0F;
            if (showLocalFog) {
    ...
    public void drawMinimap(Minecraft mc) {
        ...
        while (this.game.mainWindow.getFramebufferWidth() / (scScale + 1) >= 320 && this.game.mainWindow.getFramebufferHeight() / (scScale + 1) >= 240) {
            scScale++;
        }

        scScale += this.fullscreenMap ? 0 : this.options.sizeModifier;
        double scaledWidthD = (double) this.game.mainWindow.getFramebufferWidth() / scScale;
        double scaledHeightD = (double) this.game.mainWindow.getFramebufferHeight() / scScale;
        ...
        if (this.options.mapCorner == 1 && this.game.player.getActivePotionEffects().size() > 0) {
            int scHeight = this.game.mainWindow.getScaledHeight();
            ...
            for (PotionEffect statusEffectInstance : this.game.player.getActivePotionEffects()) {
                if (statusEffectInstance.getPotion().hasStatusIcon() && statusEffectInstance.isShowIcon()) {
            ...
            this.mapY += (int) (resFactor * statusIconOffset);
        }
    ...
    private void mapCalc(boolean full) {
        ...
        if (this.game.player.dimension == DimensionType.NETHER) {
            netherPlayerInOpen = playerChunk.canSeeSky(this.blockPos);
            nether = currentY < 126;
            ...
            boolean endPlayerInOpen = playerChunk.canSeeSky(this.blockPos);
            if (this.options.cavesAllowed && this.options.showCaves && !endPlayerInOpen) {
                caves = true;
            }
        } else if (this.options.cavesAllowed && this.options.showCaves && playerChunk.getLightFor(EnumLightType.SKY, this.blockPos) <= 0) {
            caves = true;
        }
    ...
    private int getPixelColor(
        ...
        if (needBiome) {
            if (world.getChunk(this.blockPos).isLoaded()) {
                biomeID = IRegistry.BIOME.getId(world.getBiome(this.blockPos));
            } else {
                biomeID = -1;
            }
        ...
        if (this.options.biomeOverlay == 1) {
            ...
            if (this.options.waterTransparency) {
                ...
                if (material == Material.WATER || material == Material.ICE) {
                    if (needHeightAndID) {
                        seafloorHeight = this.getSeafloorHeight(world, startX + imageX, startZ + imageY, surfaceHeight);
            ...
            if (this.options.blockTransparency) {
                if (needHeightAndID) {
                    ...
                    if (transparentHeight == -1 && this.options.waterTransparency && seafloorHeight > 0) {
                        Chunk chunk = world.getChunk(this.blockPos.withXYZ(startX + imageX, 64, startZ + imageY));
                        transparentHeight = chunk.getTopBlockY(Type.OCEAN_FLOOR, this.blockPos.getX() & 15, this.blockPos.getZ() & 15) + 1;
                        if (transparentHeight == seafloorHeight) {
                            transparentHeight = -1;
                        }
                    }
                    ...
                    if (transparentHeight != -1) {
                        ...
                        blockState = world.getBlockState(this.blockPos);
                        if (transparentHeight == surfaceHeight && blockState.getMaterial() == Material.ICE) {
                            blockState = BlockRepository.air.getDefaultState();
                        }
                    } else {
                        blockState = BlockRepository.air.getDefaultState();
                ...
                if (!needHeightAndID) {
                    ...
                    if (foliageHeight == -1 && this.options.waterTransparency && seafloorHeight > 0 && transparentHeight != seafloorHeight + 1) {
                        foliageHeight = seafloorHeight + 1;
                        this.blockPos.setXYZ(startX + imageX, foliageHeight - 1, startZ + imageY);
                        foliageBlockState = world.getBlockState(this.blockPos);
                        Material materialx = foliageBlockState.getMaterial();
                        if (materialx == Material.AIR
                                || materialx == Material.BUBBLE_COLUMN
    ...
    private final int getBlockHeight(boolean nether, boolean caves, World world, int x, int z) {
        ...
        int height = chunk.getTopBlockY(Type.LIGHT_BLOCKING, this.blockPos.getX() & 15, this.blockPos.getZ() & 15) + 1;
        if ((nether || caves) && height > playerHeight) {
            int y = this.lastY;
            this.blockPos.setXYZ(x, y, z);
            IBlockState blockState = world.getBlockState(this.blockPos);
            if (blockState.getOpacity(world, this.blockPos) == 0 && blockState.getMaterial() != Material.LAVA) {
                while (y > 0) {
                    this.blockPos.setXYZ(x, --y, z);
                    blockState = world.getBlockState(this.blockPos);
                    if (blockState.getOpacity(world, this.blockPos) > 0 || blockState.getMaterial() == Material.LAVA) {
    ...
    private final int getSeafloorHeight(World world, int x, int z, int height) {
        Chunk chunk = world.getChunk(this.blockPos);
        int seafloorHeight = chunk.getTopBlockY(Type.OCEAN_FLOOR, this.blockPos.getX() & 15, this.blockPos.getZ() & 15) + 1;

        for (IBlockState blockState = world.getBlockState(this.blockPos.withXYZ(x, seafloorHeight - 1, z));
             blockState.getOpacity(world, this.blockPos) < 5 && blockState.getMaterial() != Material.LEAVES && seafloorHeight > 1;
    ...
    private final int getTransparentHeight(boolean nether, boolean caves, World world, int x, int z, int height) {
        int transHeight = -1;
        if (!caves && !nether) {
            transHeight = world.getHeight(Type.MOTION_BLOCKING, this.blockPos.withXYZ(x, height, z)).getY();
    ...
    private int getLight(int color24, IBlockState blockState, World world, int x, int z, int height, boolean solid) {
        ...
        if (solid) {
            ...
            Chunk chunk = world.getChunk(this.blockPos);
            int blockLight = chunk.getLightFor(EnumLightType.BLOCK, this.blockPos);
            int skyLight = chunk.getLightFor(EnumLightType.SKY, this.blockPos);
    ...
    private void renderMap(int x, int y, int scScale) {
        ...
        double guiScale = (double) this.game.mainWindow.getFramebufferWidth() / this.scWidth;
        ...
        for (Waypoint pt : this.waypointManager.getWaypoints()) {
            if (pt.isActive() || pt == highlightedPoint) {
                double distanceSq = pt.getDistanceSqToEntity(this.game.getRenderViewEntity());
    ...
    private void drawWelcomeScreen(int scWidth, int scHeight) {
        if (this.welcomeString[1] == null || this.welcomeString[1].equals("minimap.ui.welcome2")) {
            ...
            this.welcomeString[4] = "§B"
                    + this.options.keyBindZoom.func_197978_k()
                    + "§F: "
                    + I18nUtils.getString("minimap.ui.welcome5a")
                    + ", §B"
                    + this.options.keyBindMenu.func_197978_k()
                    + "§F: "
                    + I18nUtils.getString("minimap.ui.welcome5b");
            this.welcomeString[5] = "§B" + this.options.keyBindFullscreen.func_197978_k() + "§F: " + I18nUtils.getString("minimap.ui.welcome6");
            this.welcomeString[6] = "§B" + this.options.keyBindWaypoint.func_197978_k() + "§F: " + I18nUtils.getString("minimap.ui.welcome7");
            this.welcomeString[7] = "§F" + this.options.keyBindZoom.func_197978_k() + "§7: " + I18nUtils.getString("minimap.ui.welcome8");
        }
