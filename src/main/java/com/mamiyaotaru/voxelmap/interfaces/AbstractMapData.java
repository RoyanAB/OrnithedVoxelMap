...
public abstract class AbstractMapData implements IMapData {
    ...
    public class Segment {
        ...
        public Segment(AbstractMapData.Point point) {
            ...
            Biome biome = null;
            if (this.biomeID != -1) {
                biome = Registry.BIOME.get(this.biomeID);
            }

            if (biome != null) {
                this.name = biome.getName().asFormattedString();
            }
        ...
        public void morphologicallyErode(boolean horizontalBias) {
            float labelWidth = MinecraftClient.getInstance().textRenderer.getStringWidth(this.name) + 8;
