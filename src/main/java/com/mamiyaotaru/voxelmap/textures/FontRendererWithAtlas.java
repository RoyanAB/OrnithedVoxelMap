...
public class FontRendererWithAtlas extends TextRenderer implements ResourceReloadListener {
    ...
    public FontRendererWithAtlas(TextureManager renderEngine, Identifier locationFontTexture) {
        super(renderEngine, new FontStorage(renderEngine, locationFontTexture));
    ...
    private void readFontTexture() {
        ...
        try {
            fontImage = TextureUtilLegacy.readBufferedImage(
                    MinecraftClient.getInstance().getResourceManager().getResource(this.locationFontTexture).getInputStream()
            );
            ...
            if (character == 167 && textIndex + 1 < text.length()) {
                ...
                if (charIndex != -1) {
                    ...
                    if (this.strikethroughStyle) {
                        ...
                        vertexBuffer.vertex(this.posX, this.posY + this.FONT_HEIGHT / 2, 0.0).next();
                        vertexBuffer.vertex(this.posX + widthOfRenderedChar, this.posY + this.FONT_HEIGHT / 2, 0.0).next();
                        vertexBuffer.vertex(this.posX + widthOfRenderedChar, this.posY + this.FONT_HEIGHT / 2 - 1.0F, 0.0).next();
                        vertexBuffer.vertex(this.posX, this.posY + this.FONT_HEIGHT / 2 - 1.0F, 0.0).next();
                        tessellator.draw();
                    ...
                    if (this.underlineStyle) {
                        ...
                        vertexBuffer.vertex(this.posX + l, this.posY + this.FONT_HEIGHT, 0.0).next();
                        vertexBuffer.vertex(this.posX + widthOfRenderedChar, this.posY + this.FONT_HEIGHT, 0.0).next();
                        vertexBuffer.vertex(this.posX + widthOfRenderedChar, this.posY + this.FONT_HEIGHT - 1.0F, 0.0).next();
                        vertexBuffer.vertex(this.posX + l, this.posY + this.FONT_HEIGHT - 1.0F, 0.0).next();
                        tessellator.draw();
                        GLShim.glEnable(3553);
                    }
