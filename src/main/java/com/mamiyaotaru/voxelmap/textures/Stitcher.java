...
public class Stitcher {
	...
	public void addSprite(Sprite icon) {
		Stitcher.Holder holder = new Holder(icon);
	...
	public void doStitch() {
		Stitcher.Holder[] stitchHoldersArray = this.setStitchHolders.toArray(new Holder[0]);
		Arrays.sort(stitchHoldersArray);
		int stitcherHoldersArrayLength = stitchHoldersArray.length;
		if (stitcherHoldersArrayLength > 0) {
			Stitcher.Holder holder = stitchHoldersArray[0];
	...
	public static class Holder implements Comparable<Stitcher.Holder> {
		...
		public int compareTo(Stitcher.Holder compareTo) {
			if (this.getHeight() == compareTo.getHeight()) {
				...
				return this.getWidth() < compareTo.getWidth() ? 1 : -1;
			} else {
				return this.getHeight() < compareTo.getHeight() ? 1 : -1;
			}
		}
	}
		...
		public boolean addSlot(Stitcher.Holder holder) {
			...
			if (this.subSlots == null) {
				...
				if (excessHeight > 0 && excessWidth > 0) {
					if (Math.max(this.height, excessWidth) > Math.max(this.width, excessHeight)) {
