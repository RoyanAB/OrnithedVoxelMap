...
//						headBits = new ModelRenderer[]{((ModelPolarBear) model).head};
	...
	private ResourceLocation getResourceLocationForEntity(ResourceLocation resourceLocation, Entity entity) {
		try {
			if (this.randomobsOptifine) {
				...
				Object randomEntitiesProperties = this.mapProperties.get(resourceLocation.getResourcePath());
		...
		if (contact.type == EnumMobs.SHEEP) {
			...
			if (helmet instanceof ItemArmor) {
				...
				if (armorType == UNKNOWN) {
					icon0 = this.textureAtlas.getAtlasSprite("armor " + helmet.getUnlocalizedName() + " 0");
					icon1 = this.textureAtlas.getAtlasSprite("armor " + helmet.getUnlocalizedName() + " 1");
					if (icon0 == this.textureAtlas.getMissingImage()) {
	...
	private Sprite[] createUnknownArmorIcons(Contact contact, ItemStack stack, Item helmet) {
		...
		if (modelBiped != null && resourceLocation != null && GLUtils.fboEnabled) {
			...
			icon0 = this.textureAtlas.registerIconForBufferedImage("armor " + helmet.getUnlocalizedName() + " 0", armorImage0);
			icon1 = this.textureAtlas.registerIconForBufferedImage("armor " + helmet.getUnlocalizedName() + " 1", armorImage1);
			this.newMobs = true;
		...
		if (icon0 == null || icon1 == null) {
			...
			this.textureAtlas.registerFailedIcon("armor " + helmet.getUnlocalizedName() + " 0");
		}
...
//			return EnumMobs.ZOMBIEVILLAGER;
	...
	private int getArmorType(ItemArmor helmet) {
		if (helmet.getUnlocalizedName().equals("item.helmetCloth")) {
			return 0;
		} else if (helmet.getUnlocalizedName().equals("item.helmetChain")) {
			return 4;
		} else if (helmet.getUnlocalizedName().equals("item.helmetIron")) {
			return 5;
		} else if (helmet.getUnlocalizedName().equals("item.helmetGold")) {
			return 6;
		} else {
			return helmet.getUnlocalizedName().equals("item.helmetDiamond") ? 7 : UNKNOWN;
		}
	}
				...
				try {
					...
					if (contact.type == EnumMobs.GHAST
						...
						if (contact.type == EnumMobs.GHAST || contact.type == EnumMobs.GHASTATTACKING) {
							...
							String path = VoxelMapProtectedFieldsHelper.getRendersResourceLocation(render, contact.entity).getResourcePath();
