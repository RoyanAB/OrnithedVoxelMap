...
public class ColorManager implements IColorManager {
    ...
    private final ColorResolver grassColorResolver = new ColorResolver() {
        @Override
        public int getColorAtPos(Biome biome, BlockPos blockPos) {
            return biome.getGrassColor(blockPos);
        }
    };
    private final ColorResolver foliageColorResolver = new ColorResolver() {
        @Override
        public int getColorAtPos(Biome biome, BlockPos blockPos) {
            return biome.getFoliageColor(blockPos);
        }
    };
    ...
    public ColorManager(IVoxelMap master) {
        ...
        for (Object obj : IRegistry.BIOME) {
            Biome biome = (Biome) obj;
            int biomeID = IRegistry.BIOME.getId(biome);
    ...
    public void loadColors() {
        ...
        TextureAtlasSprite missing = this.game.getTextureMap().getAtlasSprite("missingno");
    ...
    public final BufferedImage getBlockImage(IBlockState blockState, ItemStack stack, World world) {
        try {
            ...
            if (GLUtils.fboEnabled) {
                IBakedModel model = this.game.getItemRenderer().getItemModelWithOverrides(stack, world, null);
    ...
    private void loadSpecialColors() {
        Iterator<IBlockState> blockStateIterator = BlockRepository.pistonTechBlock.getStateContainer().getValidStates().iterator();
        ...
        Iterator<IBlockState> var6 = BlockRepository.barrier.getStateContainer().getValidStates().iterator();

        while (var6.hasNext()) {
            IBlockState blockState = var6.next();
            int blockStateID = BlockRepository.getStateId(blockState);
            this.blockColors[blockStateID] = 0;
        }
    }
    ...
    private void loadWaterColor() {
        ...
        for (int t = 0; t < 16; t++) {
            blockState = BlockRepository.water.getDefaultState().with(BlockFlowingFluid.LEVEL, t);
        ...
        try {
            ...
            if (block == BlockRepository.redstone) {
                this.blockColorsWithDefaultTint[BlockRepository.getStateId(blockState)] = this.colorMultiplier(
                        color, this.game.getBlockColors().getColor(blockState, this.game.world, blockPos, 1) | 0xFF000000
                );
            }
            ...
            System.err.println("failed getting color: " + blockState.getBlock().getNameTextComponent().getFormattedText());
            e.printStackTrace();
            return 452984832;
        }
    }
    ...
    private void applyBuiltInShading(IBlockState blockState, int color) {
        Block block = blockState.getBlock();
        int blockStateID = BlockRepository.getStateId(blockState);
        if (block == BlockRepository.largeFern || block == BlockRepository.tallGrass) {
            this.blockColorsWithDefaultTint[blockStateID] = this.colorMultiplier(color, GrassColors.get(0.7, 0.8) | 0xFF000000);
        } else if (block == BlockRepository.reeds) {
            this.blockColorsWithDefaultTint[blockStateID] = this.colorMultiplier(color, GrassColors.get(0.7, 0.8) | 0xFF000000);
        } else {
            this.blockColorsWithDefaultTint[blockStateID] = this.colorMultiplier(
                    color, this.game.getBlockColors().getColor(blockState, null, null, 0) | 0xFF000000
            );
        }
    }
    ...
    private void checkForBiomeTinting(MutableBlockPos blockPos, IBlockState blockState, int color) {
        Block block = blockState.getBlock();
        String blockName = "" + IRegistry.BLOCK.getKey(block);
    ...
    private int tintFromFakePlacedBlock(IBlockState blockState, MutableBlockPos loopBlockPos, byte biomeID) {
        ...
        try {
            ...
            Arrays.fill(fakeBiome, IRegistry.BIOME.get(biomeID));
            Chunk chunk = world.getChunk(loopBlockPos.withXYZ(fakeX, 0, fakeZ));
            Biome[] actualBiomes = new Biome[256];
            System.arraycopy(chunk.getBiomes(), 0, actualBiomes, 0, 256);
            IBlockState actualBlockState = world.getBlockState(loopBlockPos);
            chunk.setBlockState(loopBlockPos, blockState, false);
            chunk.setBiomes(fakeBiome);
            tint = this.game.getBlockColors().getColor(blockState, world, loopBlockPos, 1) | 0xFF000000;
            chunk.setBiomes(actualBiomes);
        ...
        if (world != null) {
            ...
            if (block != null) {
                ...
                try {
                    ...
                    System.arraycopy(chunk.getBiomes(), 0, originalBiomes, 0, 256);
                    IBlockState actualBlockState = world.getBlockState(loopBlockPos);
                    chunk.setBlockState(loopBlockPos, blockState, false);
                    Biome[] fakeBiome = new Biome[256];
                    ...
                    for (int biomeID = 0; biomeID < this.sizeOfBiomeArray; biomeID++) {
                        ...
                        if (biome != null) {
                            ...
                            chunk.setBiomes(fakeBiome);
                            Arrays.fill(row, this.game.getBlockColors().getColor(blockState, world, loopBlockPos, 1) | 0xFF000000);
                            tints[biomeID] = row;
                        }
                    }

                    chunk.setBiomes(originalBiomes);
                    chunk.setBlockState(loopBlockPos, actualBlockState, false);
                    int blockStateID = BlockRepository.getStateId(blockState);
                ...
                if (tints != null) {
                    ...
                    for (int t = blockPos.getX() - 1; t <= blockPos.getX() + 1; t++) {
                        for (int s = blockPos.getZ() - 1; s <= blockPos.getZ() + 1; s++) {
                            int biomeID = 0;
                            if (live) {
                                biomeID = IRegistry.BIOME.getId(world.getBiome(loopBlockPos.withXYZ(t, blockPos.getY(), s)));
                            } else {
                                int dataX = t - startX;
                                int dataZ = s - startZ;
    ...
    private int getBuiltInBiomeTint(
        ...
        if (block == BlockRepository.redstone || BlockRepository.biomeBlocks.contains(block) || this.biomeTintsAvailable.contains(blockStateID)) {
            if (!live) {
                ...
                tint = this.game.getBlockColors().getColor(blockState, world, blockPos, 1) | 0xFF000000;
            }
        }
            ...
            if (blockStates.size() == 0) {
                ...
                if (matcher.find()) {
                    ...
                    if (block != null) {
                        ...
                        if (matching.size() == 0) {
                            matching.addAll(block.getStateContainer().getValidStates());
                        }
                    ...
                    TextureAtlasSprite compareIcon = this.game.getTextureMap().getAtlasSprite(matchTiles);
                    if (compareIcon != MissingTextureSprite.getSprite()) {
                        ArrayList<IBlockState> tmpList = new ArrayList<>();

                        for (Object obj : IRegistry.BLOCK) {
                            Block testBlock = (Block) obj;
                            UnmodifiableIterator blockState = testBlock.getStateContainer().getValidStates().iterator();

                            while (blockState.hasNext()) {
                                IBlockState blockStatex = (IBlockState) blockState.next();

                                try {
                                    IBakedModel bakedModel = blockModelShapes.getModel(blockStatex);
                                    List<BakedQuad> quads = new ArrayList<>();
                                    quads.addAll(bakedModel.getQuads(blockStatex, EnumFacing.UP, this.random));
                        ...
                        System.err
                                .println(
                                        "error getting CTM block from "
                                                ...
                                                + filePath
                                                + " "
                                                + IRegistry.BLOCK.getKey(blockStates.iterator().next().getBlock()).toString()
                                                + " "
                                                + tilePath
                                );
                        ex.printStackTrace();
    ...
    private <T extends Comparable<T>, V extends T> Set<IBlockState> parseBlockMetadata(Block block, String metadataList) {
        ...
        if (metadataList.equals("")) {
            blockStates.addAll(block.getStateContainer().getValidStates());
        } else {
            ...
            if (valuePairs.size() > 0) {
                UnmodifiableIterator var22 = block.getStateContainer().getValidStates().iterator();
                ...
                while (var22.hasNext()) {
                    ...
                    for (String pair : valuePairs) {
                        ...
                        if (propertyAndValues.length == 2) {
                            IProperty<?> property = block.getStateContainer().getProperty(propertyAndValues[0]);
                            if (property != null) {
                                ...
                                for (String value : values) {
                                    ...
                                    } else if (!blockState.get(property).equals(property.parseValue(value))) {
                                        valueIncluded = true;
                                    }
                                }
    ...
    private int parseBiomeName(String name) {
        Biome biome = IRegistry.BIOME.getOrDefault(new ResourceLocation(name));
        return biome != null ? IRegistry.BIOME.getId(biome) : -1;
    }
    ...
    private List<ResourceLocation> findResources(
        ...
        for (IResourcePack resourcePack : this.getResourcePacks(namespace)) {
            if (resourcePack instanceof FilePack) {
                Object zipFileObj = ReflectionUtils.getPrivateFieldValueByType(resourcePack, FilePack.class, ZipFile.class);
                if (zipFileObj == null) {
                    return resources;
                }
    ...
    private void processColorProperties() {
        ...
        try {
            InputStream input = this.game.getResourceManager().getResource(new ResourceLocation("optifine/color.properties")).getInputStream();
            if (input != null) {
                properties.load(input);
                input.close();
            }
        ...
        UnmodifiableIterator defaultFormat = BlockRepository.lilypad.getStateContainer().getValidStates().iterator();
    ...
    private void processColorProperty(ResourceLocation resource, String list, boolean grid, int yOffset) {
        ...
        for (int t = 0; t < numBiomesToCheck; t++) {
            Biome biome = IRegistry.BIOME.get(t);
            if (biome != null) {
                int tintMult = 0;
                ...
                for (int s = 0; s < 32; s++) {
                    if (grid) {
                        ...
                        double var2x = MathHelper.clamp(biome.getDownfall(), 0.0F, 1.0F);
                        var2x *= var1x;
                        var1x = 1.0 - var1x;
                        var2x = 1.0 - var2x;
    ...
    private Block getBlockFromName(String name) {
        try {
            ...
            return IRegistry.BLOCK.containsKey(resourceLocation) ? IRegistry.BLOCK.getOrDefault(resourceLocation) : null;
