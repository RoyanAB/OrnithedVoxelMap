...
public class ColorManager implements IColorManager {
	...
	private final ColorResolver pineColorResolver = new ColorResolver() {
		@Override
	...
	private final ColorResolver leavesColorResolver = new ColorResolver() {
		@Override
		public int getColorAtPos(Biome biome, BlockPos blockPos) {
						...
						System.err
							.println(
								"error getting CTM block from "
									+ propertiesFile.getResourcePath()
									+ ": "
									+ filePath
									+ " "
	...
	private void processColorProperties() {
		...
		for (ResourceLocation resource : this.findResources("minecraft", "/mcpatcher/colormap/blocks", ".properties", true, false, true)) {
			...
			if (source != null) {
				resourcePNG = new ResourceLocation(resource.getResourceDomain(), source);
				...
				try {
					...
					path = path.normalize();
					resourcePNG = new ResourceLocation(resource.getResourceDomain(), path.toString().replace(File.separatorChar, '/'));
					System.out.println("trying " + resourcePNG);
				}
			} else {
				resourcePNG = new ResourceLocation(resource.getResourceDomain(), resource.getResourcePath().replace(".properties", ".png"));
			}
	...
	private void processColorPropertyHelper(ResourceLocation resource, String list, boolean grid) {
		ResourceLocation resourceProperties = new ResourceLocation(resource.getResourceDomain(), resource.getResourcePath().replace(".png", ".properties"));
		Properties colorProperties = new Properties();
		int yOffset = 0;
	...
	private void processColorProperty(ResourceLocation resource, String list, boolean grid, int yOffset) {
		...
		boolean swamp = resource.getResourcePath().contains("/swamp");
		...
		for (IBlockState blockState : blockStates) {
			...
			if (swamp && previousTints == null) {
				...
				if (resource.getResourcePath().contains("grass")) {
