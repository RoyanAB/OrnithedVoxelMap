...
public class WaypointManager implements IWaypointManager {
    ...
    public void onResourceManagerReload(IResourceManager resourceManager) {
        ...
        IIconCreator iconCreator = new IIconCreator() {
            ...
            public void addIcons(TextureAtlas textureAtlas) {
                ...
                for (ResourceLocation candidate : mc.getResourceManager().getAllResourceLocations("images", asset -> asset.endsWith(".png"))) {
                ...
                Sprite markerIcon = textureAtlas.registerIconForResource(
                        new ResourceLocation("voxelmap", "images/waypoints/marker.png"), Minecraft.getInstance().getResourceManager()
                );
                Sprite markerIconSmall = textureAtlas.registerIconForResource(
                        new ResourceLocation("voxelmap", "images/waypoints/markersmall.png"), Minecraft.getInstance().getResourceManager()
                );

                for (ResourceLocation resourceLocation : images) {
                    Sprite icon = textureAtlas.registerIconForResource(resourceLocation, Minecraft.getInstance().getResourceManager());
    ...
    public void newWorld(World world) {
        if (world == null) {
            ...
            this.master.getDimensionManager().enteredDimension(world.dimension);
            DimensionContainer dim = this.master.getDimensionManager().getDimensionContainerByDimension(world.dimension);
    ...
    public void handleDeath() {
        ...
        if (this.options.deathpoints != 0) {
            ...
            this.addWaypoint(
                    new Waypoint(
                            "Latest Death",
                            thePlayer.dimension != DimensionType.NETHER ? GameVariableAccessShim.xCoord() : GameVariableAccessShim.xCoord() * 8,
                            thePlayer.dimension != DimensionType.NETHER ? GameVariableAccessShim.zCoord() : GameVariableAccessShim.zCoord() * 8,
    ...
    public void changeSubworldName(String oldName, String newName) {
        if (!newName.equals(oldName) && this.knownSubworldNames.remove(oldName)) {
            ...
            File oldCachedRegionFileDir = new File(
                    Minecraft.getInstance().gameDir, "/mods/mamiyaotaru/voxelmap/cache/" + worldNamePathPart + "/" + subWorldNamePathPart
            );
                ...
                if (!pt.name.startsWith("^")) {
                    ...
                    if (dimensionsString.equals("")) {
                        dimensionsString = dimensionsString
                                ...
                                .getDimensionManager()
                                .getDimensionContainerByIdentifier(DimensionType.getKey(DimensionType.OVERWORLD).toString())
                                .getStorageName();
                        dimensionsString = dimensionsString
                                + AbstractVoxelMap.getInstance()
                                .getDimensionManager()
                                .getDimensionContainerByIdentifier(DimensionType.getKey(DimensionType.NETHER).toString())
                                .getStorageName();
                    }
    ...
    private boolean loadWaypointsExtensible(String worldNameStandard) {
        File settingsFileNew = new File(Minecraft.getInstance().gameDir, "/voxelmap/" + worldNameStandard + ".points");
        File settingsFileOld = new File(Minecraft.getInstance().gameDir, "/mods/mamiyaotaru/voxelmap/" + worldNameStandard + ".points");
    ...
    private void loadBackgroundMapImage() {
        ...
        try {
            ...
            path = path + "/" + this.currentDimension.id;
