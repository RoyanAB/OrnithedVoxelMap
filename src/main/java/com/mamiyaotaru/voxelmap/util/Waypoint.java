...
public class Waypoint implements Serializable, Comparable<Waypoint> {
    ...
    public int getX() {
        return Minecraft.getInstance().player.dimension == DimensionType.NETHER ? this.x / 8 : this.x;
    }

    public void setX(int x) {
        this.x = Minecraft.getInstance().player.dimension == DimensionType.NETHER ? x * 8 : x;
    }

    public int getZ() {
        return Minecraft.getInstance().player.dimension == DimensionType.NETHER ? this.z / 8 : this.z;
    }

    public void setZ(int z) {
        this.z = Minecraft.getInstance().player.dimension == DimensionType.NETHER ? z * 8 : z;
    }
    ...
    public int compareTo(Waypoint arg0) {
        double myDistance = this.getDistanceSqToEntity(Minecraft.getInstance().player);
        double comparedDistance = arg0.getDistanceSqToEntity(Minecraft.getInstance().player);
