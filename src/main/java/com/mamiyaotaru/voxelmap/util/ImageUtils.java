...
public class ImageUtils {
    ...
    public static BufferedImage getFaceForGameProfile(GameProfile gameProfile) {
        ...
        Map<Type, MinecraftProfileTexture> map = minecraft.getSkinProvider().getTextures(gameProfile);
        ...
        if (resourcelocation != null) {
            ...
            if (textureObject != null) {
                image = createBufferedImageFromGLID(textureObject.getGlId());
            }
        }
    ...
    public static BufferedImage createBufferedImageFromCurrentGLImage() {
        ...
        if (size < 2147483647L) {
            ...
            GLShim.glPopMatrix();
            GLShim.glPopAttrib();
            GLShim.glViewport(0, 0, MinecraftClient.getInstance().window.getFramebufferWidth(), MinecraftClient.getInstance().window.getFramebufferHeight());
        }
    ...
    public static BufferedImage blankImage(String path, int w, int h, int imageWidth, int imageHeight, int r, int g, int b, int a) {
        try {
            InputStream is = MinecraftClient.getInstance().getResourceManager().getResource(new Identifier(path)).getInputStream();
