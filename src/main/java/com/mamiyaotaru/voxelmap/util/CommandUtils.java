...
public class CommandUtils {
    ...
    public static boolean checkForWaypoints(Text chat, String message) {
        ...
        for (String waypointString : waypointStrings) {
            ...
            chatStyle.setColor(Formatting.AQUA);
            ...
            chatStyle.setHoverEvent(new HoverEvent(net.minecraft.text.HoverEvent.Action.SHOW_TEXT, hover));
            textComponents.add(clickableWaypoint);
            count = waypointStringLocation + waypointString.length();
        }

        if (count < message.length() - 1) {
            textComponents.add(new LiteralText(message.substring(count)));
        }
    ...
    private static Waypoint createWaypointFromChat(String details) {
        ...
        try {
            ...
            if (dimensions.size() == 0) {
                dimensions.add(AbstractVoxelMap.getInstance().getDimensionManager().getDimensionContainerByDimension(MinecraftClient.getInstance().world.dimension));
            }
    ...
    public static void waypointClicked(String command) {
        boolean control = InputUtil.isKeyPressed(MinecraftClient.getInstance().window.getHandle(), InputUtil.fromName("key.keyboard.left.control").getKeyCode())
                || InputUtil.isKeyPressed(MinecraftClient.getInstance().window.getHandle(), InputUtil.fromName("key.keyboard.right.control").getKeyCode());
        ...
        if (newWaypoint != null) {
            for (Waypoint existingWaypoint : AbstractVoxelMap.getInstance().getWaypointManager().getWaypoints()) {
                if (newWaypoint.getX() == existingWaypoint.getX() && newWaypoint.getZ() == existingWaypoint.getZ()) {
                    if (control) {
                        MinecraftClient.getInstance().openScreen(new GuiAddWaypoint(null, AbstractVoxelMap.getInstance(), existingWaypoint, true));
                    } else {
            ...
            if (control) {
                MinecraftClient.getInstance().openScreen(new GuiAddWaypoint(null, AbstractVoxelMap.getInstance(), newWaypoint, false));
            } else {
    ...
    public static void sendWaypoint(Waypoint waypoint) {
        int dimension = AbstractVoxelMap.getInstance().getDimensionManager().getDimensionContainerByDimension(MinecraftClient.getInstance().world.dimension).id;
        ...
        message = message + "]";
        MinecraftClient.getInstance().openScreen(new GuiSelectPlayer(null, AbstractVoxelMap.getInstance(), message, true));
    }

    public static void sendCoordinate(int x, int y, int z) {
        String message = String.format("[x:%s, y:%s, z:%s]", x, y, z);
        MinecraftClient.getInstance().openScreen(new GuiSelectPlayer(null, AbstractVoxelMap.getInstance(), message, false));
    }
    ...
    public static void teleport(String command) {
        ...
        for (Waypoint wp : AbstractVoxelMap.getInstance().getWaypointManager().getWaypoints()) {
            if (wp.name.equalsIgnoreCase(details) && wp.inDimension && wp.inWorld) {
                ...
                int y = wp.getY() > 0 ? wp.getY() : (MinecraftClient.getInstance().player.dimension != DimensionType.THE_NETHER ? 128 : 64);
    ...
    public static int getSafeHeight(int x, int y, int z, World worldObj) {
        ...
        if (inNetherDimension) {
            ...
            y = worldObj.getTop(Type.MOTION_BLOCKING_NO_LEAVES, x, z) + 1;
        }
    ...
    private static boolean isBlockOpen(World worldObj, int par1, int par2, int par3) {
        ...
        BlockState blockState = worldObj.getBlockState(blockPos);
        Block block = blockState.getBlock();
        return block == null || !blockState.canSuffocate(worldObj, blockPos);
    }
}
