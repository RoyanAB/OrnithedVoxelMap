...
public class WorldMatcher {
    ...
    public WorldMatcher(IVoxelMap master, IPersistentMap map, World world) {
        this.master = master;
        this.map = map;
        this.world = world;
    }
    ...
    public void findMatch() {
        Runnable runnable = new Runnable() {
            ...
            final File cachedRegionFileDir = new File(Minecraft.getInstance().gameDir, "/voxelmap/cache/" + this.worldNamePathPart + "/");
            ...
            public void run() {
                ...
                MessageUtils.printDebug(
                        "player coords " + player.posX + " " + player.posZ + " in world " + WorldMatcher.this.master.getWaypointManager().getCurrentWorldName()
                );
                this.x = (int) Math.floor(player.posX / 256.0);
                this.z = (int) Math.floor(player.posZ / 256.0);
                ...
                while (!WorldMatcher.this.cancelled && (this.candidateRegions.size() == 0 || this.region.getLoadedChunks() < 5) && attempts < 5) {
                    ...
                    if (this.x != (int) Math.floor(player.posX / 256.0) || this.z != (int) Math.floor(player.posZ / 256.0)) {
                        this.x = (int) Math.floor(player.posX / 256.0);
                        this.z = (int) Math.floor(player.posZ / 256.0);
                        MessageUtils.printDebug(
            ...
            private void loadRegions(String[] subworldNamesArray) {
                ...
                this.region = new ComparisonCachedRegion(
                        WorldMatcher.this.map, this.x + "," + this.z, Minecraft.getInstance().world, this.worldName, "", this.x, this.z
                );
                MessageUtils.printDebug("going to load current region");
                this.region.loadCurrent();
                MessageUtils.printDebug("loaded chunks in local region: " + this.region.getLoadedChunks());
