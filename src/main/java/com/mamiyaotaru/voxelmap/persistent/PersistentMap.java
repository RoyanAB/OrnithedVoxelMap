...
public class PersistentMap implements IPersistentMap, IChangeObserver {
    ...
    private void newWorldStuff() {
        ...
        File oldCacheDir = new File(Minecraft.getInstance().gameDir, "/mods/mamiyaotaru/voxelmap/cache/" + worldName + "/");
        if (oldCacheDir.exists() && oldCacheDir.isDirectory()) {
            File newCacheDir = new File(Minecraft.getInstance().gameDir, "/voxelmap/cache/" + worldName + "/");
    ...
    public void getAndStoreData(
        ...
        int biomeID = 0;
        if (!chunk.isEmpty()) {
            biomeID = IRegistry.BIOME.getId(chunk.getBiome(blockPos));
        } else {
            biomeID = -1;
        }
        ...
        if (biomeID != -1) {
            ...
            if (material == Material.WATER || material == Material.ICE) {
                seafloorHeight = this.getSeafloorHeight(chunk, blockPos, startX + imageX, startZ + imageY, surfaceHeight);
            ...
            if (transparentHeight == 0 && seafloorHeight > 0) {
                transparentHeight = chunk.getTopBlockY(Type.OCEAN_FLOOR, blockPos.getX() & 15, blockPos.getZ() & 15) + 1;
                if (transparentHeight == seafloorHeight) {
                    transparentHeight = 0;
                }
            }

            if (transparentHeight != 0 && transparentHeight > surfaceHeight) {
    ...
    private final int getBlockHeight(boolean underground, Chunk chunk, MutableBlockPos blockPos, int x, int z) {
        ...
        int height = chunk.getTopBlockY(Type.LIGHT_BLOCKING, blockPos.getX() & 15, blockPos.getZ() & 15) + 1;
        if (!underground) {
            return height;
        }
        ...
        if (blockState.getOpacity(this.world, blockPos) == 0 && blockState.getMaterial() != Material.LAVA) {
            while (y > 0) {
                blockPos.setXYZ(x, --y, z);
                blockState = chunk.getBlockState(blockPos);
                if (blockState.getOpacity(this.world, blockPos) > 0 || blockState.getMaterial() == Material.LAVA) {
    ...
    private final int getSeafloorHeight(Chunk chunk, MutableBlockPos blockPos, int x, int z, int height) {
        int seafloorHeight = chunk.getTopBlockY(Type.OCEAN_FLOOR, blockPos.getX() & 15, blockPos.getZ() & 15) + 1;

        for (IBlockState blockState = chunk.getBlockState(blockPos.withXYZ(x, seafloorHeight - 1, z));
             blockState.getOpacity(this.world, blockPos) < 5 && blockState.getMaterial() != Material.LEAVES && seafloorHeight > 1;
    ...
    private final int getTransparentHeight(boolean underground, Chunk chunk, MutableBlockPos blockPos, int x, int z, int height) {
        ...
        if (underground) {
            ...
            transHeight = chunk.getTopBlockY(Type.MOTION_BLOCKING, blockPos.getX() & 15, blockPos.getZ() & 15) + 1;
    ...
    private int getLight(IBlockState blockState, Chunk chunk, MutableBlockPos blockPos, int x, int z, int height, boolean solid) {
        ...
        if (solid) {
            ...
            int blockLight = chunk.getLightFor(EnumLightType.BLOCK, blockPos) & 15;
            int skyLight = chunk.getLightFor(EnumLightType.SKY, blockPos);
    ...
    public CachedRegion[] getRegions(int left, int right, int top, int bottom) {
        ...
        ArrayList<RegionCoordinates> regionsToDisplay = new ArrayList<>();
    ...
    public void handleChangeInWorld(BlockPos pos1, BlockPos pos2) {
        ...
        if (chunk != null && chunk.isLoaded() && !chunk.isEmpty()) {
            ...
            if (isChunk) {
                if (Minecraft.getInstance().currentScreen != null && Minecraft.getInstance().currentScreen instanceof GuiPersistentMap) {
