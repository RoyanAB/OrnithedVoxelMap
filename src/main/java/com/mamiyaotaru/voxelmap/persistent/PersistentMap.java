...
public class PersistentMap implements IPersistentMap, IChangeObserver {
    ...
    Comparator<PersistentMap.RegionCoordinates> distanceSorter = new Comparator<PersistentMap.RegionCoordinates>() {
        public int compare(PersistentMap.RegionCoordinates coordinates1, PersistentMap.RegionCoordinates coordinates2) {
    ...
    public void getAndStoreData(
            AbstractMapData mapData, World world, WorldChunk chunk, MutableBlockPos blockPos, boolean underground, int startX, int startZ, int imageX, int imageY
    ) {
        int surfaceHeight = 0;
        ...
        int biomeID = 0;
        if (!chunk.isEmpty()) {
            biomeID = Registry.BIOME.getRawId(chunk.getBiome(blockPos));
        } else {
            biomeID = -1;
        }
        ...
        if (biomeID != -1) {
            ...
            if (underground) {
                ...
                transparentHeight = chunk.sampleHeightmap(Type.MOTION_BLOCKING, blockPos.getX() & 15, blockPos.getZ() & 15) + 1;
                ...
                boolean hasOpacity = surfaceBlockState.getOpacity(world, blockPos) > 0;
                ...
                if (foliageBlockState.getMaterial() == Material.SNOW) {
                ...
                if (material == Material.WATER || material == Material.ICE) {
                    ...
                    for (seafloorBlockState = chunk.getBlockState(blockPos.withXYZ(startX + imageX, seafloorHeight - 1, startZ + imageY));
    ...
    public void handleChangeInWorld(int chunkX, int chunkZ) {
        ...
        try {
            ...
            if (chunk == null || chunk.isEmpty() || this.isChunkEmpty(this.world, chunk)) {
                return;
            }
    ...
    private boolean isChunkEmpty(ClientWorld world, Chunk chunk) {
        return false;
    }
